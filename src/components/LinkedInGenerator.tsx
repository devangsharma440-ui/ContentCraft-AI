...
export function LinkedInGenerator({ onSave }: { onSave: (item: { title: string; content: string; type: string }) => void }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Failed to generate LinkedIn post')
      setResult(data)
    } catch (e: any) {
      setError(e.message)
    } finally {
